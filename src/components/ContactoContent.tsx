      ...
      <section style={{ background: "rgb(248,250,252)", padding: "64px 24px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "24px" }}>
            {infoCards.map((card) => (
              <div key={card.title} style={{ background: "white", borderRadius: "16px", padding: "32px 24px", border: "1px solid rgb(226,232,240)", textAlign: "center" }}>
                <div style={{ fontSize: "36px", marginBottom: "16px" }}>{card.icon}</div>
                <h3 style={{ fontSize: "16px", fontWeight: 700, color: "rgb(13,27,62)", marginBottom: "10px" }}>{card.title}</h3>
                {card.lines.map((line) => (
                  <p key={line} style={{ fontSize: "14px", color: "rgb(100,116,139)", margin: "4px 0" }}>
                    {line}
                  </p>
                ))}
              </div>
      ...
      <section style={{ background: "white", padding: "96px 24px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "56px" }}>
            ...
            <h2 style={{ color: "rgb(13,27,62)", fontSize: "clamp(1.4rem, 2.5vw, 2rem)", fontWeight: 800 }}>
              ¿Cómo es el proceso de instalación?
            </h2>
            <div style={{ width: "60px", height: "4px", background: "linear-gradient(90deg, #f59e0b, #fde68a)", borderRadius: "2px", margin: "12px auto 0" }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "32px" }}>
            {pasos.map((p) => (
              <div key={p.num} style={{ textAlign: "center" }}>
                <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "linear-gradient(135deg, rgb(13,27,62), rgb(22,24,83))", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", color: "#f59e0b", fontSize: "18px", fontWeight: 900 }}>
                  {p.num}
                </div>
                <h3 style={{ fontSize: "17px", fontWeight: 700, color: "rgb(13,27,62)", marginBottom: "10px" }}>
                  {p.title}
                </h3>
