...
export default function MapView({ projects, selectedProject, onSelect, mapCenter }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      const initCenter: [number, number] = selectedProject
        ? [selectedProject.lat, selectedProject.lng]
        : [20, -40];
      const map = L.map(mapRef.current, {
        center: initCenter,
        zoom: selectedProject ? 5 : 3,
        zoomControl: true,
  ...
  useEffect(() => {
    ...
    // Add new markers
    projects.forEach((p) => {
      const isSelected = selectedProject !== null && p.id === selectedProject.id;
      const tooltipHtml = `
        <div class="lion-card">
  ...
  useEffect(() => {
    ...
    markersRef.current.forEach(({ marker, id }) => {
      ...
      m.setIcon(createIcon(L, selectedProject !== null && id === selectedProject.id));
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedProject]);
