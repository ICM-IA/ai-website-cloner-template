...
export default function FaqPage() {
  ...
  return (
    <main style={{ fontFamily: 'Helvetica Now Display, Helvetica, Arial, sans-serif', color: '#efefef', background: '#101010', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      {/* ── 2-column body ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 0, maxWidth: 1260, margin: '0 auto', padding: '0 32px' }}>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 32 }}>
              {categories.map(cat => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: 14, fontWeight: isActive ? 700 : 500, color: isActive ? '#C9922A' : 'rgba(239,239,239,0.65)' }}>
                      {cat.icon} {cat.name}
                    </span>
                    <span style={{ fontSize: 11, fontWeight: 600, color: isActive ? 'rgba(201,146,42,0.6)' : 'rgba(239,239,239,0.3)', background: isActive ? 'rgba(201,146,42,0.1)' : 'rgba(255,255,255,0.04)', borderRadius: 20, padding: '1px 7px' }}>{cat.faqs.length}</span>
                  </button>
                );
              })}
            </nav>
                ...
                {cat.faqs.map((faq, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: '#1a1a2e',
                        border: `1px solid ${isOpen ? 'rgba(201,146,42,0.35)' : 'rgba(201,146,42,0.12)'}`,
                        borderRadius: 10,
                        overflow: 'hidden',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
          <button
            ...
            style={{ background: '#C9922A', color: '#101010', border: 'none', borderRadius: 6, padding: '10px 20px', fontSize: 12, fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            📅 Agendar reunión por Zoom →
          </button>
          <a href={WA_URL} target="_blank" rel="noopener noreferrer" style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: 6, padding: '10px 20px', fontSize: 12, fontWeight: 800, textDecoration: 'none', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            💬 WhatsApp
          </a>
        </div>
      </div>
      ...
      <style>{`
        ...
        .lion-hover-card:hover { transform: translateY(-4px) !important; box-shadow: 0 16px 40px rgba(0,0,0,0.4), 0 0 0 1px rgba(201,146,42,0.25) !important; }
