...
const markets: Market[] = [
  ...
  {
    name: 'Uruguay',
    subtitle: 'Punta del Este · Montevideo',
  ...
  {
    name: 'Italia',
    subtitle: 'Renta vacacional — el modelo Italia de Lion GSC',
    regionFilter: 'Europa', code: 'it',
    detail: 'Costa Amalfitana · Ragusa',
    foco: false,
          ...
          <div className="two-panel" style={{ display: 'grid', gridTemplateColumns: '280px 1fr', background: '#131326', border: '1px solid rgba(201,146,42,0.15)', borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div style={{ borderRight: '1px solid rgba(201,146,42,0.1)', overflowY: 'auto', maxHeight: 780 }}>
              ...
              {filtered.map((m) => {
                ...
                return (
                  <div
                    key={m.name}
                    onClick={() => { setSelected(m.name); setOpenStep(null); setStepsOpen(false); setOpenFaq(null); setFaqsOpen(false); }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', overflowY: 'auto', maxHeight: 780 }}>
              ...
              <div style={{ padding: '28px 36px', flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 28 }}>
                ...
                {mkt.faqs && mkt.faqs.length > 0 && (
                  ...
                  <div style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
                    {/* Header clicable */}
                    <div
                      onClick={() => { setFaqsOpen(!faqsOpen); setOpenFaq(null); }}
                      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', cursor: 'pointer' }}
                    >
                      <div>
                        <p style={{ fontSize: 10, fontWeight: 700, color: '#C9922A', letterSpacing: '0.12em', margin: '0 0 3px' }}>
                          PREGUNTAS FRECUENTES — {mkt.name.toUpperCase()}
                        </p>
                        <p style={{ fontSize: 14, fontWeight: 800, color: '#efefef', margin: 0 }}>Lo que más nos preguntan</p>
                      </div>
                      <span style={{ color: 'rgba(239,239,239,0.4)', fontSize: 20, lineHeight: 1, flexShrink: 0 }}>{faqsOpen ? '−' : '+'}</span>
                    </div>
                    {/* Preguntas */}
                    {faqsOpen && (
                      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '4px 18px 8px' }}>
                        {mkt.faqs.map((faq, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              onClick={(e) => { e.stopPropagation(); setOpenFaq(isOpen ? null : i); }}
                              style={{ borderTop: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.06)', padding: '14px 0', cursor: 'pointer' }}
                            >
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                                <p style={{ fontSize: 13, fontWeight: 700, color: isOpen ? '#C9922A' : '#efefef', margin: 0 }}>{faq.q}</p>
                                <span style={{ color: 'rgba(239,239,239,0.35)', fontSize: 16, lineHeight: 1, flexShrink: 0 }}>{isOpen ? '−' : '+'}</span>
                              </div>
                              {isOpen && (
                  ...
                  <div style={{ marginTop: 24, background: 'linear-gradient(135deg, #0d1a2e 0%, #131326 100%)', border: '1px solid rgba(201,146,42,0.2)', borderRadius: 14, padding: '32px 28px', textAlign: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                        <button
                          ...
                          style={{ background: '#C9922A', color: '#101010', padding: '12px 22px', fontSize: 13, fontWeight: 800, borderRadius: 8, border: 'none', letterSpacing: '0.04em', cursor: 'pointer' }}
                        >
              ...
              <div style={{ borderTop: '1px solid rgba(201,146,42,0.12)', padding: '20px 36px', background: 'rgba(0,0,0,0.2)', display: 'flex', gap: 12, flexWrap: 'wrap', flexShrink: 0 }}>
                ...
                <button
                  onClick={() => setShowCalendar(true)}
                  style={{ background: 'transparent', color: '#C9922A', padding: '12px 20px', fontSize: 12, fontWeight: 700, border: '1.5px solid rgba(201,146,42,0.4)', borderRadius: 6, cursor: 'pointer', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}
                >
                  Agendar reunión
                </button>
              </div>
