...
export default function ProjectsSection({ selectedMarket = 'Todos' }: ProjectsSectionProps) {
  ...
  useEffect(() => {
    ...
    setTipo('all');
    setSelected(null); // clear map card when filter changes
  }, [selectedMarket]);
        ...
        <div className="proj-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: 28, alignItems: 'start' }}>
          ...
          <div style={{ position: 'sticky', top: 80 }}>
            <div style={{ borderRadius: 14, overflow: 'visible', border: '1px solid rgba(201,146,42,0.25)', height: 660, position: 'relative' }}>
              {/* Floating card — only shown when a project is selected */}
              {selected && (
                <div style={{ position: 'absolute', bottom: 16, left: 12, right: 12, zIndex: 1000, background: 'rgba(10,10,20,0.95)', border: '1px solid rgba(201,146,42,0.4)', borderRadius: 12, padding: '14px 16px', backdropFilter: 'blur(10px)', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 2 }}>
                    <p style={{ fontSize: 14, fontWeight: 800, color: '#efefef', margin: 0 }}>{selected.name}</p>
                    <button
                      onClick={() => setSelected(null)}
                      style={{ background: 'none', border: 'none', color: 'rgba(239,239,239,0.4)', fontSize: 18, lineHeight: 1, cursor: 'pointer', padding: '0 0 0 8px', flexShrink: 0 }}
                      aria-label="Cerrar"
                    >×</button>
                  </div>
