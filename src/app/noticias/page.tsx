  ...
  return (
    <main style={{ fontFamily: 'Helvetica Now Display, Helvetica, Arial, sans-serif', color: '#efefef', background: '#101010', minHeight: '100vh' }}>
          ...
          {featured && (
            <Link href={`/noticias/${featured.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 56 }}>
              <div className="featured-hero" style={{
                ...
                {/* Left: cover */}
                <div style={{ background: featured.coverBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', minHeight: 300, padding: 32 }}>
                  <div style={{ position: 'absolute', top: 16, left: 16 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: '#C9922A', background: 'rgba(201,146,42,0.12)', border: '1px solid rgba(201,146,42,0.3)', borderRadius: 20, padding: '5px 14px', letterSpacing: '0.06em' }}>
                      🔥 Noticia destacada — Abril 2026
                    </span>
                  </div>
                  <div style={{ fontSize: 100, lineHeight: 1, marginTop: 16 }}>{featured.coverEmoji}</div>
                </div>
                {/* Right: content */}
              ...
              <div className="noticias-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                {rest.map(a => (
                  <Link key={a.slug} href={`/noticias/${a.slug}`} style={{ textDecoration: 'none' }}>
                    <div className="lion-hover-card" style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%' }}>
                      {/* Cover */}
                      <div style={{ background: a.coverBg, height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                        <div style={{ position: 'absolute', top: 12, left: 12 }}>
                          <span style={{ fontSize: 10, color: 'rgba(239,239,239,0.75)', background: 'rgba(0,0,0,0.45)', borderRadius: 20, padding: '4px 10px', backdropFilter: 'blur(8px)' }}>
                            {a.countryFlag} {a.country}
                          </span>
                        </div>
                        <div style={{ position: 'absolute', top: 12, right: 12 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: a.tagColor, background: `${a.tagColor}22`, border: `1px solid ${a.tagColor}44`, borderRadius: 20, padding: '4px 10px' }}>{a.tag}</span>
                        </div>
      ...
      <style>{`
        ...
        .lion-hover-card:hover { border-color: rgba(201,146,42,0.3) !important; transform: translateY(-2px); transition: all 0.2s; }
        .featured-hero:hover { border-color: rgba(201,146,42,0.4) !important; }
      `}</style>
